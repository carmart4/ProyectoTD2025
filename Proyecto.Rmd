...
ficheros <- data.frame( FacturaFichero = character(),
                        NombreFichero = character(),
                        stringsAsFactors = FALSE
                        )

NoLeidos <- data.frame( NombreFichero = character(),
                        stringsAsFactors = FALSE
                        )
...
# Compra <- Fecha, IdTienda, Producto, Cantidad, Precio, Importe, TipoProducto, 
...
if ((Mercadona) & (!id_factura %in% ficheros$FacturaFichero)) {
  ficheros <- ficheros %>%
                add_row(
                  NombreFichero = nombre_pdf,
                  FacturaFichero = id_factura
                )
} else if ((!Mercadona) & ((!nombre_pdf %in% NoLeidos$NombreFichero))){
    NoLeidos <- NoLeidos %>%
                add_row(
                  NombreFichero = nombre_pdf
                )
...
if ((Mercadona) & (!id_tienda %in% tienda$Id)){
  tienda <- tienda %>%
                add_row(
                  IdTienda = id_tienda,
                  Direccion = direccion,
