...
*NOTA: Mejorar la instalación e importanción de librerias para que, en caso de que ya esté instalada la libreria, solo se cargue, sin actualizarse o instalarse de nuevo. (Pensando en que un ordenador sin esas librerias)*
...
# Funcion que almacena el nombre de los ficheros que son de Mercadona, y lo que no.
...
RegistroFicheros <- function(Nombrepdf, NoLeido, FicheroLeido) {
  
  verifica <- EsMercadona(Nombrepdf)
  
  if ((verifica[[1]] == FALSE) && !(Nombrepdf %in% NoLeido$NombreFichero)) {
    NoLeido <- NoLeido %>%
    ...
    if (!id_factura %in% FicheroLeido$FacturaFichero){
  ...
  valores <- list (NoLeido = NoLeido, FicheroLeido = FicheroLeido)
...
for (f in lista_nombres_ficheros) {
  resultado <- RegistroFicheros(f, NoLeido, FicheroLeido)
...
r <- RegistroFicheros(x, NoLeido, FicheroLeido)
NoLeido <- r$NoLeido
FicheroLeido <- r$FicheroLeido

```

Una vez que ya tenemos detectados los ficheros con los que vamos a trabajar, vamos a extraer la información correspondiente de ellos. 

1. Encabezado

2. Productos, identificando el tipo de producto

```{r}
ExtraeEncabezado <- function(verifica, Tienda) {
  if (verifica[[1]]){
    lineas <- verifica[[2]]
    ...
    # La información leida anteriormente, la concatenaremos en el df de tiendas. 
    # Solo si el id_tienda no está.
    
    #info_tienda <- c(id_tienda, direccion, CP, municipio, telefono)
    
    if (!id_tienda %in% Tienda$IdTienda){
          Tienda <- Tienda %>%
                        add_row(
                          IdTienda = id_tienda,
                          Direccion = direccion,
...
Tienda <- ExtraeEncabezado(verifica = verifica, Tienda)
