...
```{r}
ExtraeEncabezado <- function(lineas, Tienda) {
...
RegistroFicheros <- function(Nombrepdf, NoLeido, FicheroLeido, Tienda) {
  ...
  if ((verifica[[1]] == FALSE) && !(Nombrepdf %in% NoLeido$NombreFichero)) {
    ...
    Tienda = ExtraeEncabezado(lineas, Tienda)
    
  }
  valores <- list (NoLeido = NoLeido, FicheroLeido = FicheroLeido, Tienda = Tienda)
...
r <- RegistroFicheros(x, NoLeido, FicheroLeido, Tienda)
NoLeido <- r$NoLeido
...
lineas[indice_encabezado:indice_total]
...
DetectaParking <- function(ListaProductos){
  ...
  if (any(grepl(x=ListaProductos, pattern='PARKING')[1])) {
    parking <- TRUE
    indice_parking <- grepl(x=ListaProductos, pattern='PARKING')[1]
    productos <- ListaProductos[1:(indice_parking-1)]
  }
  valores <- c(ListaProductos, parking)
  return(valores)
}
