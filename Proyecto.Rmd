...
EsMercadona <- function(NombreFichero){
  carpeta <- 'data/'
  nombre_pdf <- paste0(carpeta, NombreFichero)
...
```{r}
# Funcion para almacenar los ficheros leidos.

# Le paso el dataframe de los ficheros leidos y no leidos. 

RegistroFicheros <- function(f, NoLeido, FicheroLeido) {
  
  if (!EsMercadona(f)[[1]] && !(f %in% NoLeido$NombreFichero)) {
    NoLeido <- NoLeido %>%
      add_row( NombreFichero = f )
  }
  else {
...
```{r}
# Prueba
resultado <- RegistroFicheros(fichero, NoLeido, FicheroLeido)

# Reasignar resultados a los originales
